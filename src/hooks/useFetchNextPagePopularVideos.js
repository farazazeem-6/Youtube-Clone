...
const useFetchNextPagePopularVideos = () => {
  ...
  const fetchMorePopularVideos = async (nextPageToken) => {
    ...
    const res = await fetch(
      `https://youtube.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&chart=mostPopular&maxResults=50&regionCode=US&pageToken=${nextPageToken}&key=${API_KEY}`
    );
